...
nStates <- length(unique(tar_read(target_eh)$data$state))
nRivers <- nStates # for now
first <- tar_read(target_eh)$first #apply(y, 1, function(x) min(which(x !=0)))
last <- tar_read(target_eh)$last
...
target_CMR_models_phiT_pT_psiT_playing2 = 
  tar_plan(
...
#save(modelOut_ttt, file = paste0('./models/runsOut/mcmc_ttt_', substr(Sys.time(),1,13), '.RData'))
