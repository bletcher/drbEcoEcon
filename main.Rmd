...
Without a postscript, the data are for all fish (main and trib)
```{r getTargets}
...
There are very few transitions between main and trib and we can characterize size transitions better with a growth model - run phiT_pT models separately for main and trib
```{r}
s <- eh$stateMatrix
s[s == 0] <- NA
...
if (tar_exist_objects(c("tt_modelOut_main"))) {
  ...
  (
    ...
    summarise(meanPhi = mean(mean),
              sdPhi = sd(mean),
              mean025 = mean(cur_data()[[3]]),
              mean50 = mean(cur_data()[[4]]),
              mean975 = mean(cur_data()[[5]])) %>%
    mutate(river = "main") %>%
    ungroup()
    )
...
ggplot(phi_tt_mainTrib_means, aes(state, mean50, color = river)) +
  geom_point(size = 2) +
  geom_line(size = 1) +
  geom_errorbar(aes(ymin = mean025, ymax = mean975), 
                width = 0.2,
                position = position_dodge(0.05),
                size = 0.75)

ggplot(psi_tt_mainTrib_means, aes(stateFrom, mean50, color = river)) +
  geom_point(size = 2) +
  geom_line(size = 1) +
  geom_errorbar(aes(ymin = mean025, ymax = mean975), 
              width = 0.2,
              ...
              size = 0.75) +
  facet_grid(stateTo ~ 1)
```


## phiT_pT_psiT  
Main and trib modelled together
```{r ttt, eval=TRUE}
### Read the model run into global memory
